...
sub completePageHandler {
  ...
  # hope this gets fixed in prince at some time
    ...
    # create temp files
    my $htmlFile =
      new File::Temp( SUFFIX => '.html', UNLINK => ( DEBUG ? 0 : 1 ) );
    my $errorFile =
      new File::Temp( SUFFIX => '.log', UNLINK => ( DEBUG ? 0 : 1 ) );
    ...
    if ($exit) {
        ...
        $html =~ s/\n/"\n".(sprintf "\%05d", $line++).": "/ge;
        throw Error::Simple(
            "execution of prince failed ($exit): \n\n$error\n\n$html");
    }

    my $url = Foswiki::Func::getScriptUrl(
        $baseWeb, $baseTopic, 'viewfile',
        filename => $pdfFile,
        t        => time(),
    );
...
sub toFileUrl {
    ...
    if ( $fileUrl =~
/^(?:https?:\/\/$Foswiki::cfg{DefaultUrlHost})?$Foswiki::cfg{PubUrlPath}(.*)$/
      )
    {
        $fileUrl = $1;
        $fileUrl =~ s/\?.*$//;
